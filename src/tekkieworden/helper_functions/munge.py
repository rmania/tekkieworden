...
def merge_duo_sdb_files(duo_file, sdb_file):
    """
...
def tag_tech_studies(input_df: pd.DataFrame, tech_keywords=List[str]):
    ...
    for c in cols_to_check:
        df["tech_keyword"] = df[c].str.findall(
            r"|".join(tech_keywords), flags=re.IGNORECASE
        )
...
if __name__ == "__main__":
    prepare_sdb_opleidingen_file(path=PATH_TO_RAW_DATA, file="stud")
